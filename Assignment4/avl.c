...
struct AVLnode * _balance(struct AVLnode * current)
{
	...
	if(rotation > 1){
		if(h(current->right->right) >=  h(current->right->left)){
			current = rotateLeft(current);
		}
		else if(h(current->right->right) <  h(current->right->left)){
			current->right = rotateRight(current->right);
			current = rotateLeft(current);
		}
	}
	else if(rotation < -1){
		if(h(current->left->left) >=  h(current->left->right)){
			current = rotateRight(current);
		}
		else if(h(current->left->left) <  h(current->left->right)){
...
{	
	if(current == NULL){
		current = (struct AVLnode *)malloc(sizeof(struct AVLnode));
